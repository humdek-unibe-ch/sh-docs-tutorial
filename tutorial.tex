...
\begin{itemize}
    ...
    \item Because of the previous point a navigation page cannot be displayed as a menu in the header.
...
Note by switching to page type \texttt{Navigation} the field \texttt{Header Position} disappears due to the fact that it cannot be used as a header menu.
Also note that the pattern \Colorbox{light-gray}{\lstinline[columns=fixed]|[i:nav]|} is appended to the \texttt{Url Pattern}.
...
There exist two possibilities to generate links to a navigation section:
\begin{enumerate}
    \item When using a link style (e.g. styles \texttt{link} or \texttt{button}), the field \texttt{url} can be set to a pattern of the specific form
        ...
        \emph{This feature is available from version v1.0.0 (January 2019).}
...
\begin{enumerate}
    \item The \texttt{POST} protocol needs to be enabled when creating a new page.
        This is done by checking the box \emph{User Input} in the \emph{Create New Page} form (see Figure~\ref{fig.create_page}).
...
\begin{enumerate}
    ...
    \item Data is stored incrementally.
        ...
        This way of storing the data is intended of log-like forms where a user is asked to enter a specific set of data every day.
\end{enumerate}
