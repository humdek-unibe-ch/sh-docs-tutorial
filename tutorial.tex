...
\begin{description}
    ...
    \item[Open Access]
        If set, the page will be accessible without having to log in to the WebApp.
    \item[Advanced]
...
%==============================================================================
\chapter{Symbols with Special Meaning}\label{sec.symb}
...
\begin{itemize}
    \item In any field of any style the following keywords can be used:
        ...
        \end{description}
    \item In the styles \texttt{form}, \texttt{link}, \texttt{button}, and \texttt{markdown} links can be generated if a special syntax is used:
        \begin{description}
    ...
    \item The field \texttt{text\_md} of style \texttt{markdown} allows the keyword \Colorbox{light-gray}{\lstinline[columns=fixed]|@<form_name>#<field_name>|} to refer to the user data of a specific input field \Colorbox{light-gray}{\lstinline[columns=fixed]|<field_name>|} of a specific form \Colorbox{light-gray}{\lstinline[columns=fixed]|<form_name>|}.
\end{itemize}
...
However, In order to fetch user input and display it on the screen either of the styles \texttt{showUserInput} or \texttt{markdown} can be used.
As described in Chapter~\ref{sec.symb} the field \texttt{text\_md} of style \texttt{markdown} allows the keyword \Colorbox{light-gray}{\lstinline[columns=fixed]|@<form_name>#<field_name>|} to refer to the user data of a specific input field \Colorbox{light-gray}{\lstinline[columns=fixed]|<field_name>|} of a specific form \Colorbox{light-gray}{\lstinline[columns=fixed]|<form_name>|}.
This allows to customize the way how user data is rendered given that the style \texttt{markdown} is very versatile in this respect.

When using the style \texttt{showUserInput} all that needs to be done is creating a section with style \texttt{showUserInput} and set the field \texttt{source} to the \texttt{name} of the form from which the data should be displayed.
